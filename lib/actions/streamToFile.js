...
exports.process = async function (msg, cfg) {
  ...
  const attachmentContent = await attachmentClient.makeRequest({
    method: 'GET',
  ...
  debug('received attachment content: %j', attachmentContent);
  ...
  const result = await s3.putObject(s3Input).promise();
